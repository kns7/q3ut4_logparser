...
/**
 * Base class that represents a row from the 'bombs' table.
 *
 *
 *
 * @package    propel.generator..Base
 */
abstract class Bombs implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\BombsTableMap';
    ...
    /**
     * Initializes internal state of Base\Bombs object.
     */
    ...
    /**
     * Compares this with another <code>Bombs</code> instance.  If
     * <code>obj</code> is an instance of <code>Bombs</code>, delegates to
     * <code>equals(Bombs)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|Bombs The current object, for fluid interface
     */
    ...
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[BombsTableMap::COL_ID] = true;
        }
    ...
    public function setPlayerId($v)
    {
        ...
        if ($this->player_id !== $v) {
            $this->player_id = $v;
            $this->modifiedColumns[BombsTableMap::COL_PLAYER_ID] = true;
        }
    ...
    public function setEvent($v)
    {
        ...
        if ($this->event !== $v) {
            $this->event = $v;
            $this->modifiedColumns[BombsTableMap::COL_EVENT] = true;
        }
    ...
    public function setWeek($v)
    {
        ...
        if ($this->week !== $v) {
            $this->week = $v;
            $this->modifiedColumns[BombsTableMap::COL_WEEK] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : BombsTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : BombsTableMap::translateFieldName('PlayerId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : BombsTableMap::translateFieldName('Week', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildBombsQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
    ...
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildBombsQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                BombsTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(BombsTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(BombsTableMap::COL_PLAYER_ID)) {
            $modifiedColumns[':p' . $index++]  = 'player_id';
        }
        if ($this->isColumnModified(BombsTableMap::COL_EVENT)) {
            $modifiedColumns[':p' . $index++]  = 'event';
        }
        if ($this->isColumnModified(BombsTableMap::COL_WEEK)) {
            $modifiedColumns[':p' . $index++]  = 'week';
        }

        $sql = sprintf(
            'INSERT INTO bombs (%s) VALUES (%s)',
    ...
    {
        $pos = BombsTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Bombs'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Bombs'][$this->hashCode()] = true;
        $keys = BombsTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(BombsTableMap::COL_ID)) {
            $criteria->add(BombsTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(BombsTableMap::COL_PLAYER_ID)) {
            $criteria->add(BombsTableMap::COL_PLAYER_ID, $this->player_id);
        }
        if ($this->isColumnModified(BombsTableMap::COL_EVENT)) {
            $criteria->add(BombsTableMap::COL_EVENT, $this->event);
        }
        if ($this->isColumnModified(BombsTableMap::COL_WEEK)) {
            $criteria->add(BombsTableMap::COL_WEEK, $this->week);
        }
    ...
    {
        $criteria = ChildBombsQuery::create();
        $criteria->add(BombsTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Bombs (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Bombs Clone of current object.
     * @throws PropelException
     */
    ...
    {
        return (string) $this->exportTo(BombsTableMap::DEFAULT_STRING_FORMAT);
    }
